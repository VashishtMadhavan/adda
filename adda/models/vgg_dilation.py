...
def vgg_dilation_scope(weight_decay=0.0005):
...
@register_model_fn('vgg_dilation')
def vgg_16_dilation_fcn(inputs,
                 num_classes=19,
                 is_training=True,
    ...
    with ExitStack() as cm:
        cm.enter_context(slim.arg_scope(vgg_dilation_scope()))
        sc = cm.enter_context(tf.variable_scope(scope, 'vgg_dilation_16', [inputs]))
        ...
        net = slim.conv2d(net, 4096, [7, 7], padding='VALID', scope='fc6', rate=4)
...
vgg_16_dilation_fcn.default_image_size = None  # fully convolutional
...
def main(gpu):
    ...
    net, end_points = vgg_16_dilation_fcn(inputs)
